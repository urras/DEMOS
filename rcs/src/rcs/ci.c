/*
 *              Запись версии в архивный файл RCS
 */
 static char rcsid[]=
 "$Header: /usr/users/korotay/DEMOS_2.2/rcs/src/rcs/RCS/ci.c,v 1.5 1986/03/05 11:33:57 dvolodin Exp $";
/*******************************************************************
...
/* $Log: ci.c,v $
 * Revision 1.5  1986/03/05  11:33:57  dvolodin
 * см. 1.4. Добавлено то же самое при вопросе "reuse prev mesg ?".
 *
 * Revision 1.4  86/03/04  15:44:52  avg
 * Добавлено уточнение имени файла в сообщении о
 * log message при работе ci с ключем -q.
 *
 * Revision 1.3  86/02/23  22:10:46  avg
 * Сделаны изменения для МНОС и для работы с U-кодом.
 *
 * Revision 1.1  85/12/26  21:55:19  fox
...
#ifdef MNOS
		owner= (StrictLocks==false) &&
		       ((getuid() & 0377) == (statbuf.st_uid & 0377));
#else
 ...
 */
{
	...
	if (logyet) {
		...
		/* otherwise ask */
		fputs("reuse log message of previous file", stdout );
		if( quietflag )printf( " for %s", workfilename );
		fputs("? [yn](y): ",stdout);
		cin=getchar();
		response=cin;
	...
	if (isatty(fileno(stdin))) {
		fputs("enter log message", stdout );
		if( quietflag )printf( " for %s", workfilename );
