...
/* $Header: /usr/users/korotay/DEMOS_2.2/rcs/src/rcs/RCS/rcskeep.c,v 1.3 1986/02/23 22:18:46 avg Exp $ */
...
/* $Log: rcskeep.c,v $
 * Revision 1.3  1986/02/23  22:18:46  avg
 * Сделаны изменения для МНОС и для работы с U-кодом.
 *
 * Revision 1.2  85/12/29  15:04:54  fox
 * Убран rcsid
 *
 * Revision 1.1  85/12/26  22:09:37  fox
...
{    register int dotcount;
     ...
     while(*sp) {
	if (*sp=='.') dotcount++;
	elsif (ctab[(unsigned)*sp]!=DIGIT) return false;
	sp++;
     }
