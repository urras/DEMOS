...
char *argv[];
{
	...
	char *getenv();
	mktemp(dirfile);
	if( !(magtape = getenv("TAPE"))) magtape = tapename;
	if (argc < 2) {
usage:
...
rep:
	*buf = 0;
	printf("Using a disk instead of tape!! disk size(kb)?");
	gets(buf);
	if ( *buf) { maxdisk = atol(buf);
		     if(maxdisk <=0) goto rep;
		}
	}
#endif
	switch(command) {
...
checkdone:
	...
	case 'R':
...
#ifndef STANDALONE
		if (command == 'R') {
			printf("Enter starting volume number (0 to block check): ");
			if (gets(tbf) == EOF) {
				volno = 1;
				printf("\n");
			}
		...
		if (readhdr(&spcl) == 0) {
			...
			if(volno == 0) goto ragain;
			exit(1);
		}
		if (volno != 0 && checkvol(&spcl, volno) == 0) {
...
char *b;
{
	...
	if (bct >= NTREC) {
		...
		bct = 0;
		if (maxdisk && (ldisk += NTREC) > maxdisk ) i=0; else
...
loop:
			flsht();
			close(mt);
			printf("Mount next volume (%d) and enter <RETURN>", volno);
			while (getchar() != '\n')
				;
			...
			if (volno && checkvol(&tmpbuf, volno) == 0) {
