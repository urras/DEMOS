/*
 * $Log:	sigjcl.c,v $
 * Revision 22.2  90/11/12  19:17:08  root
 * Новые вещи для СМ1425 и перемещение include.
 * 
 * Revision 22.1  89/04/12  15:33:14  korotaev
 * "param.h" ==> <sys/param.h>
 * 
 * Revision 22.0  89/03/30  16:31:18  korotaev
 * Begining DEMOS 2.2.
 * 
 * Revision 1.6  89/01/07  20:50:52  korotaev
 * Слияние с исправлениями насчет FIFO, +fcntl, + разные режимы открытия
 * 
 * (from VAX/UTS SV.2)
 * 
 * Revision 1.5  88/03/23  13:35:33  korotaev
 * Состояние после слияния с АЗЛК, Э-85 и Бурковским планировщиком
 * 
 * Revision 1.4  87/12/09  16:34:29  andrew
 * 31 оверлей
 * 
 * Revision 1.3  87/11/14  15:21:05  andrew
 * choverlay вызывался БЕЗ АРГУМЕНТОВ
 * 
 * Revision 1.2  87/09/23  16:03:22  root
 * Изменения связанные с truncate
 * 
 * Revision 1.1  86/04/19  15:51:32  avg
 * Initial revision
 *
 */
...
procxmt()
{
	...
	case 6:
		...
		if ((p == &u.u_ovdata.uo_curov) && ((ipc.ip_data >= 0) &&
		    (ipc.ip_data <= ((u.u_exdata.ux_mag==A_MAGIC7)?NXOVL:NOVL)) &&
