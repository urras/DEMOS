...
int	resvar;
{
	...
	/* Исправили ошибку: если || идет modify, то инфорация о отношении
	 * в конце захватов изменяется, и ее нужно читать заново. Поэтому
	 * закрываем описатель, чтобы открыть его только после блокировки
	 * Руднев А.П.
	 */
			closer1(j);
		}
