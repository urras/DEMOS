...
#	endif
	...
	rubinname(desp->reldum.relid, desp->reldum.relowner, relfile);

	/* don't loose old file before verifying new file */
	if (stat(temprel, &sbuf) >= 0)
	{
		unlink(file_name(relfile));        /* Ok if failure */
		errno = 0;
		if (link(temprel, file_name(relfile)) == -1)
		...
		if(stat(file_name(relfile), &sbuf) < 0 || !Batch_recovery)
	...
	{
		rubinname(desp->reldum.relid, desp->reldum.relowner, temprel);
		if ((i = open("admin", 2)) < 0)
		...
		if (aflag)
		{
			...
			rubinname(Admin.adattd.reldum.relid, Admin.adattd.reldum.relowner, temprel);
