...
int             F_fac, Mn_pages, Mx_pages, Rl_fill;
...
struct modtab
{
	...
	int	mn_pages;
	int	mx_pages;
	int     u_factor;       /* Умолчание. -1 - не бывает */
	char    dozero;         /* Признак - установи S_NOZERO */
};


struct modtab	Modtab[] =
{
	/* type         spec    keys    sort    seq     ffac    min  max u_factor ZERO */

	"heap",         M_HEAP, FALSE,  FALSE,  FALSE,  0,      0,      0,  -1, FALSE,
	"cheap",        -M_HEAP,FALSE,  FALSE,  FALSE,  0,      0,      0,  -1, FALSE,
	"ohash",        M_HASH, TRUE,   TRUE,   FALSE,  50,     10,     -1,  0, FALSE,
	"ochash",       -M_HASH,TRUE,   TRUE,   FALSE,  75,     1,      -1,  0, FALSE,
	"hash",         M_HASH, TRUE,   TRUE,   FALSE,  50,     10,     -1,  0, TRUE,
	"chash",        -M_HASH,TRUE,   TRUE,   FALSE,  75,     1,      -1,  0, TRUE,
	"isam",         M_ISAM, TRUE,   TRUE,   FALSE,  80,     0,      0,  -1, FALSE,
	"cisam",        -M_ISAM,TRUE,   TRUE,   FALSE,  100,    0,      0,  -1, FALSE,
	"heapsort",     M_HEAP, TRUE,   TRUE,   TRUE,   0,      0,      0,  -1, FALSE,
	"cheapsort",    -M_HEAP,TRUE,   TRUE,   TRUE,   0,      0,      0,  -1, FALSE,
	"truncated",    M_TRUNC,FALSE,  FALSE,  FALSE,  0,      0,      0,  -1, FALSE,
	"shash",        M_HASH, TRUE,   TRUE,   FALSE,  50,     10,     -1,  1, TRUE,
	"cshash",       -M_HASH,TRUE,   TRUE,   FALSE,  75,     1,      -1,  1, TRUE,
	0
};
...
#	endif
	...
	*/
	...
	rubinname(dold.reldum.relid, dold.reldum.relowner, Mod_info.infile);
...
DESC    *d;
{
    ...
    register int	err;
    char	*p2;
    int	fill_flag, min_flag, max_flag, u_flag;
    int	tupsinpg;        /* Макс. число записей в листе */
    int	ufactor;
    err = 0;
    u_flag = fill_flag = min_flag = max_flag = FALSE;
    ufactor = mp->u_factor;
    tupsinpg =  PGSIZE / (d->reldum.relwid + 2);
    /* Для компактных отношений предполагаем, что запаковка вдвое */
    if (tupsinpg < 10 && d->reldum.relspec < 0) 
        tupsinpg *= 2;
# ifdef xZTR2
    if (tTf(36, 12) )
        printf("getfill: tupsinpg = %d\n", tupsinpg);
# endif
...
min_p:
    ...
    /* Rl_fill вычисляем здесь, так как дальше tupsinpg теряется */
    if (ufactor <= 0 )
        Rl_fill = 0; /* Старый вариант */
    else
        Rl_fill = tupsinpg / ufactor;
    if (err)
        return (error(err, rel, p1, 0));
# ifdef xZTR2
    if (tTf(36, 12)  )
        printf("getfill: Rl_fill = %d\n", Rl_fill);
# endif
    return (0);
}
...
register DESC	*desc;
{
	...
	savestat = desc->reldum.relstat;
	saveprim = desc->reldum.relprim;
	desc->reldum.relspec = odesc->reldum.relspec;
	desc->reldum.relstat = odesc->reldum.relstat;
	desc->reldum.relprim = odesc->reldum.relprim;
...
# endif
	...
	desc->reldum.relstat = savestat;
	desc->reldum.relprim = saveprim;
	i = fork();
	if (i == -1)
		syserr("SORTREL: fork");
	if (i == 0)
...
# ifdef xZTR2
		if (tTf(36, 0))
		{
			...
			printf("\ttTf = `%s'\n", iocv(tTf(37, -1)));
			printf("\tformfile = `%s'\n", Mod_info.formfile);
			printf("\tinfile = `%s'\n", file_name(Mod_info.infile));
			printf("\toutfile = `%s'\n", Mod_info.outfile);
		}
# endif
		execl(buf, buf, Fileset, iocv(tTf(37, -1)),
			Mod_info.formfile, file_name(Mod_info.infile),
			Mod_info.outfile, 0);
...
# endif
			...
			*/
			...
			if (keywid > space_left(&dirbuf))
			{
				...
				dirbuf.linetab[0] = (short) (dirbuf.firstup - (char *) &dirbuf);
				offset = dirbuf.linetab[0];
				dirbuf.bufstatus = BUF_DIRECT|BUF_DIRTY;
				dirbuf.nxtlino = 0;
			}
	...
	/* На всякий случай сбросим буфера отношения */
	flush_rel(d,1);
	return (0);
}
/*
...
register DESC	*desc;
{
	...
	if (Mod_info.spflag)
	{
		...
		{
# ifdef xZTR2
# define S1(n) tt[n] = tT[n]; tT[n] = -1;
# define R1(n) tT[n] = tt[n];
			if (tTf(36, 4))
			{
				if ( nrec == 0 )
				{
				S1(27); S1(22); S1(26); S1(29);
				}
				if ( nrec == 2 )
				{
				R1(27); R1(22); R1(26); R1(29);
				}
# if 0
				if ( !tTf(36,6) && nrec == 16874)
				{
					printf("Aborted by ALEX on %d\n",nrec);
					i = 0;
					break;
				}
# endif
