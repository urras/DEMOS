...
register QTREE	*e;
{
	...
	switch (e->sym.type)
	{
	  ...
	  case BOP:
		op = e->sym.value.sym_op.opno;
		if (op == opCONCAT || op == opDACHAR || op == opTICHAR ||
		    op == opTODA || op == opTOTI || op == opINIT )
		{
			pr_op(BOP,op);
			pr_expr(e->left);
			printf(", ");
...
register QTREE	*c;
{
	...
	switch (c->sym.type)
	{
	  case INT:
		...
		break;
	  case DATE:
		printf("%d",date_char(c->sym.value.sym_data.i4type,NULL));
		break;
	  case TIME:
		printf("%d",time_char(c->sym.value.sym_data.i4type,NULL));
		break;
	  case FLOAT:
		if (c->sym.len == 4)
