...
SCCSID(@(#)ksort.c	7.3	8/24/83)

# define	N	7
# if defined(MC68000) || defined(s_386)
#  define        MEM    2048*1024
# else
#  define        MEM     (32768 - 2)
# endif
# define	BUCKETSIZE	4
...
**              0400    Трассировка восстановления пустых листов
*/
...
char	**argv;
{
	...
	** value for tuple */
	...
	{
	 char *s, *getenv();int mm,i;
	 if ( (s = getenv("KSORTMEM")) != (char *)0 )
	 {
	     if ( (mm = atoi(s)) != 0 && mm >= 20 && mm <= 16000 ) {
		    mem = mm * 1024l;
		    printf("Ksort7: mem=%dk\n",mem/1024);
		    }
	  }
	}
...
**      Attention: Недай бог вам здесь попытаться зайти на flushpage
*/
...
struct tup_id	*tid;
{
	...
	if (b->thispage != pageid)
	{
		...
		if ((d->reldum.relstat & S_NOZERO) && pageid+1 < d->reldum.relprim)
		{
			if (b->mainpg == 0)
			{
				mk_zpage(b);
			if (Trace&0400)
				printf("Восстановлен лист page=%ld на %.14s\n", pageid,d->reldum.relid);
			}
			if (b->mainpg == MAINZERO)
			{
				b->mainpg = 0;
			if (Trace&0400)
				printf("Восстановлен 0 в page=%ld на %.14s\n", pageid,d->reldum.relid);
			}
		}
		Accuread++;
	}
