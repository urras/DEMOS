			...
			if ($1)
			{
				if (!Fieldp && ! Cvarp)
				{
					if (!Field_indir && !Indir_level
					  && ( sequal($1->d_elm, "dba")
					    || sequal($1->d_elm, "sysdate")
					    || sequal($1->d_elm, "systime")
...
afcn:		aggrfcn	 
	...
	|       id lparen afcn rparen %prec FOP
	|       id lparen afcn comma afcn rparen %prec FBOP
;
