...
**  to the RUBIN standard.  2) The changes marked by comments.
*/
...
/* -------- the next line is an RUBIN customization -------- */
...
yyparse()
{
	...
	/* RUBIN customization to make 'ps', 'p', and 'n' register variables */
	int		s[MAXDEPTH];
	register int	*ps, *p;
	register int	n;
	int		ac;
...
actn:		/* get the next action, and perform it */
	...
	switch (ac >> 12)	/* switch on operation */
	{
	  ...
	  case 1:	/* skip on test */
		...
		/* ---------- the next two lines are an RUBIN customization ---------- */
		if (yychar < 0)
			return(1);
		if (n != yychar)
			p++;
	  ...
	  case 3:		/* reduce */
		...
		yypv =- yyr2[n];
		yyval = yypv[1];
		/* --------  the next 2 lines are an RUBIN customization -------- */
		if (yypflag && yyactr(n))
			 goto abort;
