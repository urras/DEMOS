...
SCCSID(@(#)creatdb.c	7.7	9/26/83)

/*
**  CREATDB -- создание базы данных или изменение статуса
**
**      Программа создает новую базу. Она берет имя базы (синтакис ниже),
**      создает такую базу и устанавливает флаги.
**
**      Выполнить команду можно только при условии, что установлен бит
**      U_CREATDB в параметрах пользователя в файле users.
**
**      Флаг -m используется, если справочник для базы данных уже
**      существует на своем месте (в data/base). Этот ключ позволяет
**      использовать в качестве базы данных отдельную файловую систему,
**      Справочник должен быть пуст.
**
**      Флаг -e служит для изменения статуса существующей базы.
**      Сама база данных должна уже существовать и быть корректной.
**
**      Флаг -dимя_справочника позволяет указать нестандартное размещение
**      базы данных. В этом случае в справочнике data/base создается
**      файл под именем базы данных, в котором записано действительное
**      расположение базы данных - имя справочника из параметра -d... .
**        Обращение:
**		creatdb [flags] databasename
**
**      Позиционные параметры:
**              databasename -- имя базы данных - не более 14 букв и цифр,
**                      начинаться имя должно с буквы.
**
**      Флаги:
**		-m
**                      База - это монтируемая файловая система.
**                      Справочник для базы "data/base/имя" долженh
**                      существовать и быть пустым.
**		-e
**                      База уже существует, нужно только изменить
**                      режимы доступа и флаги.
**		-uXX
**                      Вызвать команду от имени пользователя XX. Этот
**                      ключ можно использовать только от имени супер-
**                      пользователя РУБИН. Пользователь XX становится
**                      администратором создаваемой базы (DBA).
**		-Fpathname
**                      Использовать pathname как файл template.
**		+-c
...
#	endif
	...
	*/
	...
	switch (i)
	{
	  ...
	  case 3:
		printf("You are not a valid RUBIN user\n");
		exit(-1);

	  case 4:
		printf("No database name specified\n");
	usage:
		printf("Usage: creatdb [-ddir_name] [-uname] [-e] [-m] [+-c] [+-q] dbname\n");
		exit(-1);
 ...
 */                     fread(&direct, sizeof (struct direct), 1, stdin);
			fread(&direct, sizeof (struct direct), 1, stdin);
			while ( fread(&direct, sizeof (struct direct), 1, stdin) != EOF)
			{
				if ( direct.d_ino != 0)
					syserr(0, "%s is not empty", database);
			}
...
#	endif
	...
	rubinname(p++->pv_val.pv_str, Usercode, fname);
	...
	for (; p++->pv_type != PV_EOF; p++)
	{
		...
		des->reldum.relwid += R_SKIP(des->reldum.relwid,c,len);
...
register RELDES	*r;
{
	...
	rubinname(r->parmv[1].pv_val.pv_str, Usercode, d.reldum.relid);
#	ifdef xSTR1
	if (tTf(12, 0))
		printf("creat %s\n", d.reldum.relid);
#	endif
...
readadmin()
{
	...
	if (flagval('e'))
	{
		...
		rubinname("relation", Admin.adhdr.adowner, relname);
		if ((Admin.adreld.relfp = open(relname, 2)) < 0)
			syserr("readadmin: open `%.14s'", relname);
		rubinname("attribute", Admin.adhdr.adowner, relname);
...
/*
 * Mkpointer(Path) - под именем Dbpath создать ссылку на Path
 */
Mkpointer(Path)
char	*Path;
{
...
err: 
    perror(Dbpath);
    syserr("Can not create data-base-pointer %s", Dbpath);
}
