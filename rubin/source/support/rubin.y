...
**			site in a distributed rubin. (Used in dist.
**			rubin' initproc() function.)
...
**			must be RUBIN or the DBA for the database to
**			use this option.
...
**			monitor).
**		.../files/users -- file with UNIX uid -> RUBIN code
...
/* bits for prstat */
# define PR_REALUID	0001		/* run as the user, not RUBIN */
...
char	**argv;
{
	...
	extern int	Status;
	char		*proctab;
	char Vers[16];  /* Код версии, [+код Q, +Код SQL ] */
	register char	*p;
	char		*ptr;
	...
	*/
	...
	if ((Admin.adhdr.adflags & A_QRYMOD) == 0)
		{
		doflag("-q", -1);
		concat(Vers,NO_QRYMOD,Vers);
		}
	/* Проверим, не SQL-RUBIN ли это */
	if (argv[0][0] == 's') concat(Vers,SQL_PROC,Vers);
...
# define        F_ACCPT1       10       /* Принять если ровно 1 символ */
# define        F_D_SPEC       11       /* Спецификация даты */
# define        F_T_SPEC       12       /* Спецификация времени */
...
struct flag	Flagok[] =
{
	...
	'c',	0,		F_C_SPEC,	0,		NULL,
	'd',    F_PLD|F_PLOK,   F_ACCPT1,       0,              NULL,
	'd',    0,              F_D_SPEC,       0,              NULL,
	'f',	0,		F_F_SPEC,	0,		NULL,
	'i',	0,		F_I_SPEC,	0,		NULL,
...
int	where;
{
	...
	for (f = Flagok; f->flagname != p[1] || (f->flagsyntx == F_ACCPT1 && p[2] != 0) ; f++)
	{
		if (f->flagname == 0)
			goto badflag;
	}
	...
	if (Nopts >= MAXOPTNS)
	{
		printf("Too many options to RUBIN\n");
		exit(1);
	}
...
**	execs the RUBIN processes.  Isn't that neat?
**
**	Parameters:
**		none
**
	...
	int procno;
{
	...
	*v = 0;
	...
	if ((pr->prstat & PR_CLSDOUT) == 0)
		closeit[2] = FALSE;
	if (Alockdes >= 0) closeit[Alockdes] = FALSE;
