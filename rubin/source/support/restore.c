...
SCCSID(@(#)restore.c	7.5	7/7/83)

/*
** RUBIN crash recovery processor
**	to recover a database you must be the dba or the rubin superuser
...
checkrel()
{
	...
	for (;;)
	{
		...
		if ((rel.relstat & (S_VIEW|S_SERIAL)) == 0)
		{
			rubinname(rel.relid, rel.relowner, fname);
			if ((j = open(file_name(fname), 2)) == -1)
			{
				printf("\tNo file for:\n\t");
				printup(&Reldes, &rel);
				if (ask("\tDelete tuple? "))
