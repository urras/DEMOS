...
char	**av;
{
	...
	/*
	** close all the files descriptors, so we can have more RUBIN
	...
	*/
	for ( ;; )
	{
		...
		*/
		...
		{
# ifdef	DEBUG
		ildebug && printf("select returns 0 (%o)\n",read_fd);
# endif	DEBUG
			read_fd = Read_fmt;
		}
# ifdef	DEBUG
		ildebug && printf("select returns %d (%o)\n",num_des,read_fd);
# endif	DEBUG
		if ( num_des == -1 )
		{
# ifdef	DEBUG
		ildebug &&         perror("DRIVER:num_des = -1");
# endif DEBUG
			/*
...
register	int	read_desc;
{
	...
	errno = 0;
# ifdef	DEBUG
	ildebug && printf("DRIVER: entering ilwrite, read_desc = %d\n",read_desc);
# endif	DEBUG
	if ( read(read_desc,&lockbuf, sizeof ( lockbuf)) != sizeof ( lockbuf ) )
	{
# ifdef	DEBUG
	ildebug &&         printf("Read error, errno = %d\n",errno);
# endif	DEBUG
...
# ifdef	DEBUG
	if (ildebug)
		ildebug && printf("ildr: act %d, type %d, mode %d, read_desc %d\n",
...
# endif	DEBUG
	...
	{
# ifdef	DEBUG
		ildebug && printf("Illegal request\n");
# endif	DEBUG
		send_info(read_desc,-5);
		return;
	}
...
# ifdef	DEBUG
			if (ildebug) {
				...
				printf("ildr: find ");
				for (i = 0; i < KEYSIZE; i++)
					printf("%d ", ll->lr_key[i]);
			       printf("\n mod= %d type= %d ",ll->lr_mod, ll->lr_type);
			}
# endif
...
# ifdef	DEBUG
	ildebug && printf("DRVIER: Removing all, key = %d\n",key);
# endif
	for ( i = 0; i < NLOCKS; i++ )
...
ilclose()
{
	...
	register caddr_t c;
# ifdef	DEBUG
	ildebug && printf("DRIVER: entered close\n");
# endif
