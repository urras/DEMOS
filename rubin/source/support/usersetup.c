...
**	probably be run once only, when RUBIN is first installed.
**	Otherwise, usercodes can change mysteriously.
**
**	The optional parameter replaces the root of the RUBIN subtree
**	as found in /etc/passwd.  The RUBIN user must be installed
**	(with that name) when usersetup is run.  If this parameter
**	is a minus ("-"), output goes to the standard output.
**
**	The initialization file is initialized to "<home>/.rubin",
...
char	**argv;
{
	...
	/* scan for RUBIN in /etc/passwd */
	while (fgets(buf, MAXLINE, iop))
	{
		i = decode(buf, field);
		if (!sequal(USERRUBIN, field[0]))
			continue;
		pathname = field[i - 1];
	...
	/* test for RUBIN entry found */
	if (!pathname)
		syserr(0, "USERRUBIN not installed as UNIX user");
	...
	while (fgets(buf, MAXLINE, iop))
	{
		i = decode(buf, field);
		/* print username & code */
		printf("%s:%s:%s:%s:%s:::%s/.rubin::\n",
			field[0],	/* user name */
			code,
			field[2],	/* user id */
			field[3],	/* user group */
			sequal(field[0], USERRUBIN) ? "177777" : stat,
