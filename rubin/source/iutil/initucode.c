...
SCCSID(@(#)initucode.c	7.4	9/26/83)
...
**	example, rubin.c cats file .../files/usage on this sort of
**	error.
**
...
**		3 -- you are not a valid RUBIN user.
...
**			the RUBIN user or the DBA can use the -u
**			flag.
**		Pathname -- set to the pathname of the RUBIN subtree.
**		Status -- an integer set to the user status field
**			of the users file for the real user.
**		Ing_uid -- set to the user id of the RUBIN user.
**
...
**
**	Files:
**		/etc/passwd -- to get the pathname for user "rubin".
...
char	*Pathname;	/* path of RUBIN subtree */
...
int	Ing_uid;	/* the user id of the RUBIN user */
...
# endif
	...
	*/
	...
	/*
	**  Get pathname of RUBIN subtree from /etc/passwd file
	**  entry for USERRUBIN (presumably "rubin") and save it
	**  in 'Pathname'.
	**
	**  This algorithm suggested by Jim Popa.
	*/
	Ing_uid = geteuid();    /* На случай INGPATH */
# ifdef xV7_UNIX
...
# endif xV7_UNIX
		...
		} while (i<3 || !sequal(buf, USERRUBIN));

		/* we now have the RUBIN passwd file entry in 'buf' */
		fclose(iop);
...
# endif xV7_UNIX
	...
	/* create the RUBIN user id */
	Ing_uid = atoi(field[2]);
# ifdef xV6_UNIX
	Ing_uid &= 0377;
# endif
# ifdef xB_UNIX
...
# endif
		...
		bmove((char *) &(ibuf.st_ino), (char *) Lock.dbnode, 4);
#endif
		retval -= 2;
