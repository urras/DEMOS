...
SCCSID(@(#)copy.c	7.6	9/26/83)

/*
**  COPY -- Performs an rubin COPY.
**
**	Trace Flags:
**		30
*/
...
int     bin_swab;               /* Swab bytes, if 1 */

int	Into;			/* into is one if this is a copy into file */

char	Inbuf[PGSIZE];		/* input holder */
char	Outbuf[PGSIZE];		/* output holder */
...
int	doff;	/* destination offset */
{
	...
	if (sf != CHAR)
	{
		if (df == CHAR)	/* numeric to char conversion */
		{
			switch (sl)
			{
			  ...
			  case 4:
				...
				{
					smove(date_char(inp->i4type,NULL), temp); /* convert and copy */
				}
				else if (sf == TIME)
				{
					smove(time_char(inp->i4type,NULL), temp); /* convert and copy */
				}
				else
				{
		...
		if(bin_swab) swab_bytes(outp,dl);
		return (0);
	}
			...
			if (Into && (mp->flen == 0) && mp->ftype == CHAR)
			{
				switch (mp->rtype)
				{
				  ...
				  case INT:
					...
					break;
				  case DATE:
					       mp->flen = date_len(NULL);
					       break;
				  case TIME:
					       mp->flen = time_len(NULL);
					       break;
				  case FLOAT:
					if (mp->rlen == 4)
...
struct map	*mp;
{
	...
	** buffer area.
	*/
	count = (mp->rtype == CHAR && mp->ftype != DUMMY) ? mp->rlen + 1 : PGSIZE;
	esc = FALSE;
...
/*
 * swab_bytes(s,l)
 * Переставить байты в строке s длины l в обратном порядке
 */
swab_bytes(s,l)
register char *s;
int l;
