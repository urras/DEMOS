...
register ANYTYPE	*value;
{
	...
	{
	  case TIME:
		printfatt(time_len(NULL),time_char(value->i4type,NULL));
		break;
	  case DATE:
		printfatt(date_len(NULL),date_char(value->i4type,NULL));
		break;
	  case INT:
		switch (length)
		{
...
/*
 * Печать элемента заголовка
 * Если length < 0, то только посчитать длину
 */
...
register char	*value;
{
	register int	i;
	char		c;
	register short Count;   /* 1 - не печатать */
	if (length < 0)
	{
	  Count = 1;
	  length = -length;
	...
	{
	  case TIME:
		length = time_len(NULL);
		break;
	  case DATE:
		length = date_len(NULL);
		break;
	  case INT:
		switch (length)
		{
