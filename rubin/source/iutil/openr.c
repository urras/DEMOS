...
**      			mode -4.
**              mode -6 -- as -1, but can open variable
**	Parameters:
**		dx - a pointer to a struct descriptor (defined in rubin.h)
...
#	endif
	...
	switch (mode)
	{
	  ...
	  case -1:
		...
		if (retval == 0 && mode != -6 && (d->reldum.relstat & S_SERIAL))
			retval = 1;
		break;
	  case 1:
	  case 2:
	  ...
	  case 0:
		retval = get_reltup(d, name);
		if (retval == 0 && (d->reldum.relstat & S_SERIAL))
			retval = 1;
		if (retval )
			break;
	  ...
	  case -4:
		...
		rubinname(d->reldum.relid, d->reldum.relowner, fname);
		/* can't open a view */
		...
		if ((d->relfp = open(file_name(fname), filemode)) < 0)
		{
