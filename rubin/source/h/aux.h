...
# define        S_NOZERO        0040000         /* Признак "дырявого" файла */
# define        S_SERIAL        0100000         /* Переменная */
...
# define	U_SUPER		0100000		/* rubin superuser */
...
/*
**	Usercode contains the current user's RUBIN user-id code.
**	Pathname contains the name of the RUBIN subtree.
*/
...
# define	MAXLINE		256
/*
 * Максимальное число переменных (SERIAL)
 * которые можно использовать одновременно
 * (точнее, которые система еще помнит одновременно)
 */
# define        MAXSERLAST   10

/* file mode for RUBIN files */
# define	FILEMODE	0600		/* db file mode */

# ifndef ROUND
# define R_SKIP(offset,fmt,len) 0
# else
# if ROUND == 2
# define R_SKIP(offset,fmt,len) ((((offset)&01)!=0 && (fmt) != CHAR && (len) != 1)?1:0)
# else
U_N_I_M_P_L_E_M_E_N_T_E_D___R_O_U_N_D
# endif
# endif
