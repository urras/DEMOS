...
#if 0
static char Sccsid[] = "$Header: interp.c,v 1.2 87/11/25 17:12:48 rubin Exp $";
#endif


/*
** $Log:	interp.c,v $
Revision 1.2  87/11/25  17:12:48  rubin
...
#	endif
	...
	byflag = (int)(list == De.ov_bylist);        /* set byflag if aggregate function */
	  ...
	  case COP:
		...
		switch (opval)
		{
		  ...
		  case opUSERCODE:
			...
			goto loop;
		 case opSYSDATE:
			tos->value.sym_data.i4type = get_date();
			tos->len = 4;
			tos->type = DATE;
			goto loop;
		 case opSYSTIME:
			tos->value.sym_data.i4type = get_time();
			tos->len = 4;
			tos->type = TIME;
			goto loop;
		}
	  ...
	  case BOP:
		...
		switch (tos->type)
		{
		  ...
		  case TIME:
			...
			{
			  case 1:
				syserr("interp: I1 type in BINOP");
			  case 2:
				switch (opval)
				{
				  case opADD:
		...
		case CHAR:
			...
			case opINIT:
			      if (val2->i4type < 0)
				      ov_err(SERNONEG);
			      l = ser_set(val1->cptype, val2->i4type);
			      if (l < 0)
				      ov_err(SERNODEF);
			      val1->i4type = l;
			      tos->type = op2->type;
	   ...
	   case UOP:
		...
		switch (opval)
		{
		  ...
		  case opINT1:
			typecoerce(tos, INT, 2); /* Обработать как i_2 */
			goto loop;
		  ...
		  case opINT4:
			...
			goto loop;
		  case opLAST:
			l = ser_last(val1->cptype);
			i = SERNOLAST;
			goto chk_ser;
		  case opNEXT:
			l = ser_set(val1->cptype, -1l);
			i = SERNODEF;
chk_ser:
			if ( l < 0)
				ov_err(i);
		  ...
		  case opFLOAT8:
			...
			goto loop;
		  case opDATE:
			typecoerce(tos, DATE, 4);
			goto loop;
		  case opTIME:
			typecoerce(tos, TIME, 4);
			goto loop;
		  default:
			if (tos->type == CHAR)
				ov_err(BADUOPC);
...
register SYMBOL	*tos;
{
	...
	switch (opval)
	{
	  ...
	  case opAVGU:
	  case opAVG:
		if (tos->type == INT || tos->type == DATE || tos->type==TIME)
			itof(tos);
		else
			if (tos->type == CHAR)
				ov_err(BADAVG);
