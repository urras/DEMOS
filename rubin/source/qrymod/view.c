...
#			endif
			...
			subsvars(&r, vn, vtree->left, mdVIEW, vtree->right);

			/* test for non-functional replace */
			if (Qt.qt_qmode == mdREPL && vn == Qt.qt_resvar && bitcnt(varset(r) | (1 << Qm.qm_newresvar)) > 1)
				qmerror(3360, Qt.qt_qmode, Qt.qt_resvar, 0);	/* non-functional update */

			/* append new qualification into qual */
