...
**
**	This module performs the RUBIN protection algorithm, as
**	presented in Stonebraker & Rubinstein, "The RUBIN Protection
...
short   byset[8];
{
	...
	int    allretr;    /* permit retrieve to all */

	t = root;
	vn = varno;
	allretr = !bitset(S_PROTRET,Qt.qt_rangev[vn].rngvdesc->reldum.relstat);
...
#	endif
	...
	if (noperm == 0 && allretr)
		return; /* Нет update и всем можно retrieve */
...
#	ifdef xQTR1
	if (tTf(51, 12))
		printf("No perm on %o\n", noperm);
#	endif
	/* Если всем можно retrieve, то не нужно на них смотреть */
	if (allretr) noperm &= ~(PRO_RETR|PRO_AGGR);
