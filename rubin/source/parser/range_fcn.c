...
# define        LOOKNEW         0       /* Дополнение к LOOKVAR, LOOKREL */
...
#	ifdef	xPTR2
	tTfp(66, 4, "rnglook:\ttype = %s\tname = %s\n",
		    (type != LOOKREL ? "variable" : "relation"), name);
...
#	endif
	...
	for (slot = 0; slot < MAXVAR; slot++, rptr++)	/* search external vbles only */
	{
		if (rptr->relvused
		    ...
		    (type != LOOKREL ? rptr->vardesc.relvname : rptr->vardesc.reldum.relid),
		    MAXNAME) == 0)
		{
...
#	endif
	...
	{
		if ((type == R_IMPLICIT) || (slot = rnglook(var, LOOKNEW)) < 0)
		{
			/* not in range table */
			slot = rngold();
		}
...
# ifdef	xPTR2
	tTfp(67, 10, "rngreset().\n");
# endif
	clrrange(); /* Старый вариант не вполне корректен */
}

/*
** CHECKUPD
		...
		switch (op)
		{
		  ...
		  case mdMODIFY:
			if (!openr(&desc, -1, rptr->vardesc.reldum.relid))
				...
				{
					bmove(rptr->vardesc.relvname, var, MAXNAME);
