...
# endif
		  ...
		  case opLAST:
			...
			if (Trfrmt != CHAR)
			{
				par_error(NEEDCHAR,WARN,0);
			}
			/* GARB!!! */
			result->left->sym.len += 1;
			/* Проваливаемся */
		  case opINT4:
			rfrmt = INT;
			rfrml = 4;
		...
		switch (result->sym.value.sym_op.opno)
		{
		  ...
		  case opDIV:
			...
			else
			{
				if (Trfrmt == DATE)
					rfrmt = DATE;
				else if (Trfrmt == TIME && rfrmt != DATE)
					rfrmt = TIME;
				if (Trfrml > rfrml)
					rfrml = Trfrml;
			 }
		  ...
		  case opMOD:
			...
			rfrml = Trfrml;
			format(result->right);
			if ((rfrmt != INT && rfrmt != DATE && rfrmt != TIME)
			 || Trfrmt != INT )
		  ...
		  case opINIT:
			...
			if (Trfrmt != CHAR)
				par_error(NEEDCHAR,WARN,0);
			format(result->right);
			if (Trfrmt != INT && Trfrmt != DATE && Trfrmt != TIME)
				par_error(NEEDPOSINT,WARN,0);
			/* GARB!!! */
			result->left->sym.len = 1+length (result->left->sym.value.sym_data.c0type);
			rfrmt = Trfrmt;
			rfrml = 4;
			break;
		  case opCONCAT:
		  ...
		  case opTOTI:
			...
			format(result->left);
			if ( Trfrmt != CHAR ||
			     (format(result->right),Trfrmt != CHAR) )
				par_error(BADTODATE,WARN,0);
			if ( time_len(result->right->sym.value.sym_data.c0type) < 0)
				par_error(BADTODATE,WARN,0);
			/* Шаблон должен кончаться нулем - увеличим длину (см. tree() )*/
			/* GARB!!! */
			result->right->sym.len = 1+length (result->right->sym.value.sym_data.c0type);
			rfrml = 4;
			break;
		  case opTODA:
			rfrmt = DATE;
			...
			if ( Trfrmt != CHAR ||
			     (format(result->right),Trfrmt != CHAR) )
				par_error(BADTODATE,WARN,0);
			if ( date_len(result->right->sym.value.sym_data.c0type) < 0)
				par_error(BADTODATE,WARN,0);
			/* Шаблон должен кончаться нулем */
			/* GARB!!! */
			result->right->sym.len = 1+length (result->right->sym.value.sym_data.c0type);
			rfrml = 4;
			break;
		   case opDACHAR:
			format(result->left);
			if (Trfrmt == CHAR)
				par_error(BADDACHAR,WARN,0);
			format(result->right);
			if(result->right->sym.type != CHAR ||
			   (rfrml = date_len(result->right->sym.value.sym_data.c0type)) < 0)
				par_error(BADDACHAR,WARN,0);
			/* Шаблон должен кончаться нулем */
			/* GARB!!! */
			result->right->sym.len = 1+length (result->right->sym.value.sym_data.c0type);
		   ...
		   case opTICHAR:
			format(result->left);
			if (Trfrmt == CHAR)
				par_error(BADTICHAR,WARN,0);
			format(result->right);
			if(result->right->sym.type != CHAR ||
			   (rfrml = time_len(result->right->sym.value.sym_data.c0type)) < 0)
				par_error(BADTICHAR,WARN,0);
			/* Шаблон должен кончаться нулем */
			/* GARB!!! */
			result->right->sym.len = 1+length (result->right->sym.value.sym_data.c0type);
