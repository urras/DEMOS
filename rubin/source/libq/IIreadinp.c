...
#	ifdef xATR1
	if (IIdebug)
		printf("IIreadinput start: type=%d\n",ppb->pb_type);
#	endif
...
#	ifdef xATR1
	if (IIdebug)
		printf("IIreadinput: ERROR %s=%d from=%d\n", pv[0],eno,ppb->pb_from);
#	endif
		IIerror(eno, pc - 1, &pv[1]);
		if(bitset(PB_INFO,ppb->pb_stat))
		{
			IIpb_prime(ppb, PB_NOTYPE);
			goto repeat_read;
		}
	}
}
...
# ifdef xATR1
		if (plen != sizeof j)
			IIsyserr("readinput: PV_INT %d", plen);
		if(IIdebug)
			printf("readarg: PV_INT %d = %s\n",j,ib);
# endif
