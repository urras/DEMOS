/*
**      Получение статистики по обменам с внешними устройствами.
**
**      iostat [-t] [-i] [-s] [-b] [-d] [-a] [интервал [число]]
**
**      Стандартное местоположение в системе:  /etc
**
**      Метод сборки:  cc -O -n -s [-f]
**
**      $Header$
**      $Log$
**
...
char   *argv[];
{
    ...
    nlist ("/demos", nl);
    if (nl[0].type == 0) {
        printf ("dk_busy not found in /demos namelist\n");
        exit (1);
    }
    mf = open ("/dev/kmem", 0);
    if (mf < 0) {
