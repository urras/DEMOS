...
**
**      Жуть!! Ее скоро переделаем, а пока она такая.
**
**
**
*/
...
int             onsusp();
#endif
int             lcaseflg;       /* Признак - есть lcase */
...
/*
** A real function, for the putch routine in termlib
*/
...
char ch;
{
    if(lcaseflg && (ch&0100))
        { if(ch >= 'A' && ch <= 'Z') ch += 040;
          else if ( ch >= '\0140' && ch <= '\0176') ch += 0140;
        }
    putchar (ch);
}
...
#ifdef RED
                pr ("red");  putchar (' ');pr (fnames[fnum]); putchar (' ');pr(cmdbuf+1);
                execute (filename, RED, "red", fnames[fnum], cmdbuf+1, 0);
#else
