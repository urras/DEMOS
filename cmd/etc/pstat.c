...
#include "../h/NBUFMOU.h"
#include "../h/NPROCTXT.h"
#include "../h/NINODFIL.h"
#include "../h/param.h"
#include "../h/conf.h"
#include "../h/tty.h"
...
dotty()
{
        ...
        read(fc, (char *)&kl11, sizeof(kl11));
        mesg = " # RAW OUT   MODE   ADDR   DEL COL  STATE   PGRP\n";
        printf(mesg);
        ttyprt(0, &kl11);
        if(setup[SNDH].type == 0)
                ...
                ttyprt1(SNDH, SDH, "dh");
        if(setup[SNDZ].type == 0)
                printf("no DZ11's\n");
        else
                ttyprt1(SNDZ, SDZ, "dz");
        if(setup[SNDC].type == 0)
                printf("no DC11's\n");
        else
                ttyprt1(SNDC, SDC, "dc");
        if(setup[SNDJ].type == 0)
                printf("no DJ11's\n");
        else
                ttyprt1(SNDJ, SDJ, "dj");
}
...
char *s;
{
        register struct tty dh11[48];
        int ndh;
        register struct tty *tp;

        lseek(fc, (long)setup[num].value, 0);
        read(fc, (char *)&ndh, sizeof(ndh));
        printf("%d %s lines\n", ndh, s);
        lseek(fc, (long)setup[type].value, 0);
