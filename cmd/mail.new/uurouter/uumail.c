 ...
 * Теперь неопознанные имена машин в UUCP не сваливаются на uux,
 * а просто отсылаются.
 * 
 * Revision 2.27  91/01/10  21:42:58  avg
 * Добавлено отрезание локального имени в bang path и поправлено
 * слияние адресов с путем вида aaa!%bbb
 * 
 * Revision 2.26  91/01/05  23:52:02  avg
 * Добавлено преобразование в bang path при передаче c флагом F.
 * 
 * Revision 2.25  90/12/12  21:53:00  avg
 * Добавлено выдирание имени машины-источника сообщения из
 * UU_MACHINE если нет remote from...
 * 
 * Revision 2.24  90/12/10  17:08:52  avg
 * Добавлен вызов batchmail и ключ -a при вызове uux.
 * 
 * Revision 2.23  90/11/15  18:36:43  avg
 * Добавлено откидывание локального домена.
 * 
 * Revision 2.22  90/11/12  19:11:33  avg
 * При отсылке адрес берется из поля Sender, а не из From
 * если есть оба.
 * 
 ...
 * Добавлена обработка нескольких адресатов в .forward
 * 
 * Revision 2.18  90/09/26  23:14:08  avg
 * Добавлена обработка Return-Receipt-To.
 * 
 * Revision 2.17  90/09/26  20:17:46  avg
 * Добавлено сопоставление пустого со *.
 * 
 ...
 * Добавлено приписывание локального uucp-шного имени в таблицу.
 * 
 ...
 * я забыл добавлять .uucp к локальным link-ам!
 * ю
 * 
 ...
 * Добавлена хитрая обраьотка домена .uucp и правлен формат даты
 * в ARPA.
 * 
 * Revision 2.7  90/08/20  22:42:57  avg
 * Добавлен флаг -r к uux-у (отменяется указанием /I в Systems).
 * 
 * Revision 2.6  90/08/18  15:53:49  avg
 * Починено сравнение с шаблонами в режиме manual-routing
 * Правлена обработка конструкций a%b%c@d
 * 
 ...
 * Добавлен (пустой) ключ -o...
 * 
 * Revision 2.2  90/07/29  23:07:36  avg
 * Массовая правка мелких плюх.
 * 
 * Revision 2.1  90/07/29  20:51:35  avg
 * Добавлена работа с таблицами в формате автоматического маршрутизатора.
 * Теперь uumail поддерживает сервисные программы, вызывая их как
 * 	uu-router -r откуда
 * Добавлен сбор статистики по передачам почтовых сообщений.
 * 
 * Revision 1.29  90/07/19  17:41:28  avg
 * Правлена ошибка в ifdef-е.
 * 
 * Revision 1.28  90/07/02  00:33:41  avg
 * Сделан флаг mailer-а /S - strip host.domain part.
 * 
 * Revision 1.27  90/06/30  22:34:25  avg
 * Добавлена более подробная диагностика невозможности рассылки
 * локальным пользователям.
 * Сделана проверка числа пересылок для предотвращения зацикливания.
 * 
 ...
 * Теперь обрабатывает вложенные () в полях заголовка.
 * 
 * Revision 1.23  90/06/21  23:17:38  avg
 * Добавлено поле Date в Error Report.
 * 
 * Revision 1.22  90/06/20  16:07:26  avg
 * Правлена ошибка в alias-ах - uumail не заходил на aliasы
 * для имен вида user@localhost.
 * 
 * Revision 1.21  90/06/19  22:30:12  avg
 * Теперь uumail НИКОГДА не передает флаг -r - это очень странная штука.
 * 
 * Revision 1.20  90/06/19  13:35:40  avg
 * Добавлено закрытие лишних файлов.
 * Немного изменена стратегия счета корелляции.
 * 
 * Revision 1.19  90/06/18  16:51:14  avg
 * Правлена передача аргументов для uux.
 * 
 * Revision 1.18  90/06/17  19:55:05  avg
 * *** empty log message ***
 *       ALIAS
 * Revision 1.17  90/06/17  19:50:46  avg
 * Забывал удалить \n из конца дурацкой строки From ...
 * 
 * Revision 1.16  90/06/17  19:44:44  avg
 * Немного правлена диагностика - теперь uumail отличает явные
 * uucp-шные адреса от остальных.
 * 
 * Revision 1.15  90/06/17  19:38:35  avg
 * Правлена пара мелких плюшек.
 * 
 * Revision 1.14  90/06/17  17:50:15  avg
 * Правлена ошибка при склеивании длинных длинных адресов.
 * 
 * Revision 1.13  90/06/12  22:27:13  avg
 * Сделано просекание локального адреса посылателя.
 * 
 * 
 * Revision 1.12  90/06/11  14:56:13  avg
 * Если нет sendmail-а, то для приема ошибок теперь используется
 * rmail.
 * 
 * Revision 1.11  90/06/11  14:49:33  avg
 * Правлена обработка глупой строки 'From'.
 * Добавляется Received: by ....
 * Улучшена диагностика ошибок.
 * 
 * Revision 1.10  90/06/10  18:47:05  avg
 * Теперь для отсылки ошибок и локальной почты вызываются разные
 * mailer-ы.
 * 
 * Revision 1.9  90/06/08  21:35:43  avg
 * Теперь uumail при посылке сообщения через uucp всегда добавляет
 * строку From ... remote.
 * Кроме того, описатель маршрута @ для локальной машины добавляется
 * всегда по умолчанию.
 * (Internet-овское имя машины должно быть в файле /etc/myhostname
 * если нет команды hostname).
 * 
 * Revision 1.8  90/06/08  16:21:49  avg
 * Добавлена возможность отсылки сообщений об ошибках
 * без ключа -r.
 * 
 * Revision 1.7  90/06/08  15:31:20  avg
 * Добавлена обработка alias-ов.
 * 
 ...
 * Добавлена правильная отработка плохих кодов возврата
 * uux.
 * 
 * Revision 1.3  90/06/05  16:44:45  avg
 * Убран (пока) ключ -h  и добавлена обраотка явного пути в форме
 * @a,@b,@c:d@e		(аналогично @a:@b:@c:d@e)
 * 
 ...
 * Правлен вызов uux так, что в аргументах rmail адреса будут
 * указываться в скобках для отмены специального значения ! для uux.
 * 
...
#ifndef lint
static char rcsid[] = "$Header: uumail.c,v 2.28 91/01/12 17:18:16 avg Exp $";
#endif
...
TryAgain:
	 ...
	 * get route for it
	 *
	 * Здесь было бы неплохо сделать перекручивание:
