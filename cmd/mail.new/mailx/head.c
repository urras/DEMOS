 ...
 *
 * $Log:	head.c,v $
 * Revision 1.12  90/12/22  22:54:39  ache
 ...
 * Теперь не падает на неверном заголовке.
 * 
 ...
 * Правлена функция raise на предмет русских букв.
 * 
...
/*NOXSTR*/
static char rcsid[] = "$Header: head.c,v 1.12 90/12/22 22:54:39 ache Exp $";
/*YESXSTR*/
static void fail();
...
"нет ни поля from ни поля date"));
		return(0);
	}
	if (!isdate(hl.l_date)) {
		fail(linebuf, ediag(
"date field not legal date",
"в поле date неверная дата"));
		return(0);
	}
	...
	register int c;
{
	...
	return c >= 'a' && c <= 'z'
#ifndef MSDOS
	    || c >= 0300 && c <= 0337  /* КОИ-8, russian letters */
	    || c == 0243               /* Yo */
#else
	    || c >= 0xa0 && c <= 0xaf
	    || c >= 0xe0 && c <= 0xef
