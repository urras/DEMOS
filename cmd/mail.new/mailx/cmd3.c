...
/*NOXSTR*/
static char rcsid[] = "$Header: /usr/src/relcom/mailx/RCS/cmd3.c,v 1.15 1991/01/25 18:04:45 ache Exp $";
/*YESXSTR*/
...
"Нельзя ответить на несколько сообщений сразу\n"));
		return(1);
	}
...
"Пустое поле reply-to -- ответим автору\n"));
		if (cp == NOSTR)
			strcpy(buf, rcv);
		else
			strcpy(buf, cp);
	...
	head.h_bcc = NOSTR;
	head.h_refs = NOSTR;
	if( (cp = hfield("message-id", mp)) != NULL ) {
		cp2 = salloc(strlen(cp)+1);
		strcpy(cp2, cp);
		head.h_refs = cp2;
	}
	head.h_resent = 0;
