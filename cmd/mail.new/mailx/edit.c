...
/*NOXSTR*/
static char rcsid[] = "$Header: /usr/src/relcom/mailx/RCS/edit.c,v 1.13 1991/01/25 18:04:45 ache Exp $";
/*YESXSTR*/
extern char *udate();
...
ed);
		...
		BadFrom:
				printf(ediag("WARNING: wrong started message -- corrected\n",
					     "ПРЕДУПРЕЖДЕНИЕ: неверно начатое сообщение -- исправлено\n"));
		...
		if (!lastline) {
			printf(ediag("WARNING: wrong ended message -- corrected\n",
				    "ПРЕДУПРЕЖДЕНИЕ: неверно законченое сообщение -- исправлено\n"));
			putc('\n', tf);
			ms++;
#ifdef  MSDOS
			ms++;
