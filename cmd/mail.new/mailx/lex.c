...
/*NOXSTR*/
static char rcsid[] = "$Header: /usr/src/relcom/mailx/RCS/lex.c,v 1.15 1991/01/25 18:04:45 ache Exp $";
/*YESXSTR*/
	...
	char *name;
{
	...
	maketemp(tempMesg);
#endif
	if ((i = creat(tempMesg, 0600)) < 0) {
		perror(tempMesg);
		exit(1);
	}
	close(i);
	if ((tf = TmpOpen(tempMesg, "a+")) == NULL) {
		perror(tempMesg);
		remove(tempMesg);
		exit(1);
	}
...
"Нельзя выполнять \"%s\" -- файл сообщений можно только читать\n"),
		   com->c_name);
		if (loading)
			return(1);
		if (sourcing)
...
"Неверное использование \"списка сообщений\"\n"));
			return(-1);
		}
...
"%s воспринимает не больше чем %d аргументов\n"),
	...
	default:
		panic(ediag("Unknown argtype","Неизвестный тип аргументов"));
	}
...
setmsize(sz)
{
	...
	if (msgvec == NULL)
		panic(ediag("No memory for mesg list","Нет памяти для списка сообщений"));
}
...
char    *r_greeting       = "Почта. Наберите ? для подсказки.\n";
...
newfileinfo()
{
	...
	printf("\"%s\": ", ename);
	if (msgCount == 0)
		printf(ediag("no messages","нет сообщений"));
	else if (msgCount == 1)
		printf(ediag("1 message","1 сообщение"));
	else if (msgCount > 0)
		printf(ediag("%d messages","%d сообщений"), msgCount);
	if (n > 0)
