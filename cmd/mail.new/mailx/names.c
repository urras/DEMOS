...
/*NOXSTR*/
static char rcsid[] = "$Header: /usr/src/relcom/mailx/RCS/names.c,v 1.21 1991/01/25 18:04:45 ache Exp $";
/*YESXSTR*/
...
"detract запрашивает вставку запятых\n"));
	...
	struct header *hp;
{
	...
	while (np != NIL) {
		...
		if (image == NULL) {
			...
			if ((image = TmpOpen(tempEdit, "a+")) == NULL) {
				perror(tempEdit);
				remove(tempEdit);
				senderr++;
				goto cant;
...
#endif
				puthead(hp, image, GTO|GSUBJECT|GCC|GNL);
			}
			rewind(fo);
