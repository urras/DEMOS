...
/*NOXSTR*/
static char rcsid[] = "$Header: /usr/src/relcom/mailx/RCS/fio.c,v 1.15 1991/01/25 18:04:45 ache Exp $";
/*YESXSTR*/
	...
	FILE *ibuf;
{
	...
	maketemp(tempSet);
#endif
	close(creat(tempSet, 0600));
#ifndef MSDOS
	if ((mestmp = TmpOpen(tempSet, "a+")) == NULL) {
#else
	if ((mestmp = TmpOpen(tempSet, "a+b")) == NULL) {
#endif
		perror(tempSet);
		remove(tempSet);
		exit(1);
	...
	if ((s = value("sort")) != NOSTR) {
		...
		default:
			fprintf(stderr, ediag("WARNING: bad sort order, must be one of the date, subject, author\n",
					      "ПРЕДУПРЕЖДЕНИЕ: порядок сортировки должен быть date, subject или аuthor\n"));
			return;
		}
...
edstop()
{
	...
	if (stat(editfile, &statb) >= 0 && statb.st_size > mailsize) {
		...
		if ((obuf = TmpOpen(tempBack, "a+")) == NULL) {
			perror(tempBack);
			remove(tempBack);
			relsesigs();
			reset(0);
