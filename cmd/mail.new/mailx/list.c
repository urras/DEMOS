 ...
 *
 * $Log:	list.c,v $
 * Revision 1.9  90/11/11  20:06:21  ache
 ...
 * Поправлена обработка кавычек в getrawlist в сторону shell-style.
 * 
 * Revision 1.5  90/06/10  00:43:17  avg
 * Сделано сопоставление с образцом при поиске сообщений в команде from.
 * 
...
/*NOXSTR*/
static char rcsid[] = "$Header: list.c,v 1.9 90/11/11 20:06:21 ache Exp $";
/*YESXSTR*/
...
"Номера не должны быть вместе со *\n"));
				return(-1);
			}
			mc++;
			other++;
...
"Неверная последовательность \"%s\" после ':'\n"),
					    lexstring);
					return(-1);
				}
				colmod |= colresult;
...
"Нельзя, чтобы вместе были \"*\" и что-то еще\n"));
				return(-1);
			}
			star++;
			break;
...
{
	if (++regretp >= REGDEP)
		panic(ediag("Too many regrets","Слишком много regret'ов"));
...
mark(mesg)
{
	...
	if (i < 1 || i > msgCount)
		panic(ediag("Bad message number to mark",
				"Плохой номер сообщения для mark"));
...
unmark(mesg)
{
	...
	if (i < 1 || i > msgCount)
		panic(ediag("Bad message number to unmark",
				"Плохой номер сообщения для unmark"));
	message[i-1].m_flag &= ~MMARK;
}
