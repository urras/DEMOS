...
/*NOXSTR*/
static char rcsid[] = "$Header: /usr/src/relcom/mailx/RCS/quit.c,v 1.9 1991/01/19 15:38:23 ache Exp $";
/*YESXSTR*/
...
"Пришла новая почта.\n"));
		...
		maketemp(tempResid);
#endif
		close(creat(tempResid, 0600));
		if ((rbuf = TmpOpen(tempResid, "a+")) == NULL) {
			perror(tempResid);
			remove(tempResid);
		}
	...
	if (p == msgCount && !modify && !anystat) {
		if (p == 1)
			printf(ediag(
"Held 1 message in %s\n",
"1 сообщение остается в %s\n"),
mailname);
		else if (p > 0)
			printf(ediag(
"Held %d messages in %s\n",
"%d сообщений остаются в %s\n"),
p, mailname);
		TmpDel(rbuf);
		unlock();
		return;
	...
	if (value("append") == NOSTR) {
		...
		maketemp(tempQuit);
#endif
		close(creat(tempQuit, 0600));
		if ((ibuf = TmpOpen(tempQuit, "a+")) == NULL) {
			perror(tempQuit);
			remove(tempQuit);
			TmpDel(rbuf);
			unlock();
...
"1 сообщение записывается в mbox\n"));
	else if (mcount > 0)
		printf(ediag(
"Saved %d messages in mbox\n",
"%d сообщений записываются в mbox\n"),
mcount);
