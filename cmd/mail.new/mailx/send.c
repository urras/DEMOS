...
/*NOXSTR*/
static char rcsid[] = "$Header: /usr/src/relcom/mailx/RCS/send.c,v 1.30 1991/01/25 18:50:28 ache Exp $";
/*YESXSTR*/
...
"неудача с обработкой поля status\n"));
	return(lc);
}
...
"Ни сообщения, ни темы; наверное так и надо\n"));
		    else
			printf(ediag(
"Null message body; hope that's ok\n",
"Пустое сообщение; наверное так и надо\n"));
		if ((mtf = infix(hp, mtf)) == NULL) {
			fprintf(stderr, ediag(
". . . message lost, sorry.\n",
". . . сообщение пропало, извините.\n"));
			return(-1);
		}
	}
...
"Должны быть вставлены запятые в список получателей\n"));
	...
	FILE *fi;
{
	...
	maketemp(tempMail);
#endif
	close(creat(tempMail, 0600));
	if ((nf = TmpOpen(tempMail, "a+")) == NULL) {
		perror(tempMail);
		remove(tempMail);
		TmpDel(fi);
		return NULL;
	}
	puthead(hp, nf, GTO|GSUBJECT|GCC|GNL|GREFS|GADD);
	while ((c = getc(fi)) != EOF)
		putc(c, nf);
	if (ferror(fi)) {
		ioerror("temp", 0);
