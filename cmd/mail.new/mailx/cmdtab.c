 ...
 *
 * $Log:	cmdtab.c,v $
 * Revision 1.9  90/12/20  20:30:40  ache
 * top перенесена выше (нужнее, чем type)
 * 
 ...
 * Добавлены команды @Reply @reply @from @headers.
 * 
 * Revision 1.3  90/05/31  19:45:40  avg
 * Поменялись местами команды reply и Reply. Добавлена команда headline.
 * 
...
/*NOXSTR*/
static char rcsid[] = "$Header: cmdtab.c,v 1.9 90/12/20 20:30:40 ache Exp $";
/*YESXSTR*/
...
extern int XRespond(), Xrespond(), Xheaders(), Xfrom(), Forward();
#ifdef PMCS
extern int projects();
#endif
