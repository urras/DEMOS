 ...
 *
 * $Log:	optim.c,v $
 * Revision 1.12  90/09/29  18:23:13  ache
 * <ctype.h> kicked out...
 * 
 ...
 * Добавлена версия gethostname для XENIX 286 by nms
 * 
...
/*NOXSTR*/
static char rcsid[] = "$Header: optim.c,v 1.12 90/09/29 18:23:13 ache Exp $";
/*YESXSTR*/
static void optim();
static void optim1();
...
"неизвестная машина %s, используется: %s\n"),
cp, name);
		return(name);
	}
...
"машина %s известная, но удаленная, используется: %s\n"),
			    cp, name);
		return(name);
	}
	strcpy(newname, name);
...
"машина локальная, вызов netmap(%s, %s)\n"),
		    newname, fakepath);
...
"Нет места для идентификаторов машин\n"));
		return(0);
	}
	if (xp->xh_name == NOSTR) {
		if (midfree >= XHSIZE) {
			printf(ediag(
"Out of machine ids\n",
"Слишком много идентификаторов машин\n"));
			return(0);
		}
		xtab[midfree] = xp;
...
"Использование полученного идентификатора неопределенной машины\n"));
		return(NOSTR);
	}
...
"ОШИБКА:  неизвестный внутренний идентификатор машины\n"));
		return(0);
	}
	if ((stype & dtype) == 0)
		return(0);
...
"Нельзя найти свободного места для динамического входа главной\n"));
		exit(1);
	}
	np->nt_machine = host;
}
