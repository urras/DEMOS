...
#endif HAYES
#ifdef HAYES2400
	{ "ACU", "hayes2400", Acuopn, hys24popn, hyscls24 },
	{ "ACU", "hayes2400pulse", Acuopn, hys24popn, hyscls24 },
	{ "ACU", "hayes2400tone", Acuopn, hys24topn, hyscls24 },
#endif HAYES2400
...
#endif ATT2224
#ifdef MNP5
	{ "ACU", "mnp5tone", Acuopn, mnp5topn, mnp5cls },
	{ "ACU", "mnp5pulse", Acuopn, mnp5popn, mnp5cls },
#endif MNP5
...
register char *flds[];
{
	...
	while (rddev(dfp, &dev) != FAIL) {
		...
		if (!cd->CU_brand) {
			/* brand name not found - it can be external dialer */
			DEBUG(6, "Dialer: %s called\n", dev.D_brand);
			acustatus++;
			fd = extdopen(phone, flds, &dev);
			if (fd > 0)
				CU_end = extdclose;   /* point CU_end at close func */
		} else {
			DEBUG(4, "Using %s\n", cd->CU_brand);
			acustatus++;
