...
#if defined(BSD2_9) && defined(BSDTCP)
#define NOGETPEER
#endif
...
#ifdef  NOSTRANGERS
		if (chklogsys(Loginuser, Rmtname)) {
			/* If we don't know them, we won't talk to them... */
			logent(Rmtname, "BAD LOGIN");
			omsg('R', "Unknown login to this machine", Ofn);
			cleanup(0);
		}
#endif	NOSTRANGERS
