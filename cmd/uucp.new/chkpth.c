...
/*
 *	this routine will check the path table for the
 *      machine and log name. This logname must be defined
 *      for this machine in USERFILE.
 *
 *	return codes:  0  |  FAIL
 */

chklogsys(logname, mchname)
char *mchname, *logname;
{
	register struct userpath *u;
	extern char *lastpart();
	register char **p, *s;
	...
	for (u = Uhead; u != NULL; ) {
		if (strcmp(logname, u->us_lname) == SAME
			&& strncmp(mchname, u->us_mname, MAXBASENAME) == SAME)
			return SUCCESS;
		u = u->unext;
	}
	return FAIL;
