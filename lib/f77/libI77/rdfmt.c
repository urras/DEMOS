...
#include "fmt.h"
extern int cursor;
rd_ed(p,ptr,len) register char *ptr; register struct syl *p; ftnlen len;
{       register int ch;
...
rd_ned(p,ptr) register char *ptr; register struct syl *p;
{
	switch(p->op)
	{
...
rd_A(p,len) register char *p; ftnlen len;
{       register int i,ch;
	for(i=0;i<len;i++)
	{	GET(ch);
		*p++=VAL(ch);
...
rd_AW(p,w,len) register char *p; ftnlen len;
{       register int i,ch;
	if(w>=len)
	{	for(i=0;i<w-len;i++)
			GET(ch);
