/*
 * 21.11.86 Добавлен параметр в (*donewrec)('\n')
 * 21.11.86 Исправлена ошибка - пропущена база в аргументе icvt в wrt_IM
 * Руднев
 */
#include "fio.h"
#include "fmt.h"
...
w_ed(p,ptr,len) register char *ptr; register struct syl *p; ftnlen len;
{
...
w_ned(p,ptr) register char *ptr; register struct syl *p;
{
	switch(p->op)
	{
...
{       int ndigit,sign,spare;register int i;
	long x;
	char *ans;
...
wrt_IM(n,w,m,len) register uint *n; ftnlen len;
...
wrt_H(a,b) register a;
{       register char *s=(char *)b;
	if(mv_cur()) return(mv_cur());
	while(a--) (*putn)(*s++);
	return(1);
}
wrt_L(n,len) register ftnint *n;
{       register int i;
	for(i=0;i<len-1;i++)
		(*putn)(' ');
	if(*n) (*putn)('t');
...
wrt_A(p,len) register char *p; ftnlen len;
{
	while(len-- > 0) (*putn)(*p++);
	return(0);
}
wrt_AW(p,w,len) register char * p; ftnlen len;
{
	while(w>len)
	{	w--;
		(*putn)(' ');
...
{	double up = 1,x;
	int oldscale=scale,n,j; register i;
...
{       int delta,dp,sign,n;register i;
	double x;
	char *s,*fcvt();
	x= (len==sizeof(float)?p->pf:p->pd);
	if(scale)
