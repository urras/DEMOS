...
int     i;
{
	...
	if (i >= 0) {
#ifdef RUSMAC
		if (contab[i].mask & MMASK)
#else
		if (contab[i].rq & MMASK)
#endif RUSMAC
			ffree (((filep) contab[i].x.mx) << BLKBITS);
		contab[i].rq = 0;
		contab[i].x.mx = 0;
#ifdef RUSMAC
		contab[i].mask = 0;
...
int     mn;
{
	...
	apptr = (filep) 0;
#ifdef RUSMAC
	if (app && (oldmn >= 0) && (contab[oldmn].mask & MMASK)) {
#else
...
filep p;
{
	...
	if ((i = (p & (~(BLK - 1)))) != roff) {
		...
		if (read (ibf, (char *) rbuf, BLK * sizeof (int)) == 0)
			return (0);
	}
	return (rbuf[p & (BLK - 1)]);
}
...
filep newip;
{
	...
	if ((enda - sizeof (struct s)) < (char *) nxf)
		                                setbrk (DELTA);
	p = nxf;
	p -> pframe = frame;
	p -> pip = ip;
...
/*      lim = (int *) (nxf = savnxf + sizeof (struct s) / sizeof (savnxf)); */
	strflg = 0;
	if ((argppend =
		(argpp = (int **) savnxf + (sizeof (struct s)
				/ sizeof (int **)))
			+ (sizeof (struct s) - 1))
		> (int **) enda)
			setbrk (DELTA);
	strp = (int *) argppend;
	lim = (int *) (nxf = (struct s *) ((char *) strp + DELTA));
	for (i = 8; i >= 0; i--)
		argpp[i] = 0;
