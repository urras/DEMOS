 ...
 */
	switch (k) {
		...
		case 'e': 
		/* printable version of current eschar */
			i = PRESC;
			goto gx;
...
char    ifilt[32] = {
	0, 01, 02, 03, 0, 05, 06, 07, 010, 011, 012
};
...
#else
	if ((copyf == 0) && ((i & ~0777) == 0) && ((i & CMASK) < 0477)) {
#endif  ASA
		i |= chbits;
	}
