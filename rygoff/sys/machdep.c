/*
 * $Log:	machdep.c,v $
 * Revision 1.3  90/02/21  16:00:41  rygoff
 * При инициализации таймера
 * можно выбрать разные частоты прерываний
 * (частота задается в param.c константой HZ для Э-85)
 * Вероятно, такой могучий таймер, как на Э-85
 * можно использовать еще более эффективно.
 * 
 * Revision 1.2  90/02/21  15:55:38  rygoff
 * Инициализация таймера Э-85
 * Введено отличие от Micro-PDP 11/23:
 * NVI > 0
 * 
 * Revision 1.1  90/02/21  15:51:59  rygoff
 * Initial revision
 * 
...
clkstart()
{
	...
	if (fioword((caddr_t)lks) == -1) {
		...
		extern int hz; /* Set in param.c by constant HZ */
		register int i=16, j=hz;
		if(j>8192 || j<2) hz=64;
		while( j>>=1 ) --i; /* Select correct setting for freq. */

#               define rB(x)  *((char *)x)
#               define rW(x)  *((int  *)x)

		rW(TIM85_A) = i|040;    /* Set frequency & protection */
