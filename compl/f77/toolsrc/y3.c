				...
				TLOOP(k){
					if( BIT(u->ws.lset,k) ){
						  if( temp1[k] == 0 ) temp1[k] = c;
						    ...
						    if( foutput!=NULL )
						      fprintf( foutput, "\n%d: конфликт св/св ( свертки %d и %d ) при %s",
...
apack(p, n ) int *p;{ /* pack state i from temp1 into amem */
	...
	for( rr=amem; rr<=r; ++rr,++off ){  /* try rr */
		...
		for( qq=rr,pp=p; pp<=q; ++pp,++qq ){
			if( *pp ){
				if( qq > r )
				   error( "Нет места в таблице действий" );
				if( qq>memp ) memp = qq;
				*qq = *pp;
				}
			}
...
"\n%d: конфликт сдв/св (сдвиг %d, свертка %d) при %s",
...
wrstate(i){ /* writes state i */
	...
	TLOOP(j0) if( (j1=temp1[j0]) != 0 ){
		...
		if( j1>0 ){ /* shift, error, or accept */
			...
			else fprintf( foutput,  "сдвиг\t%d", j1 );
			}
		else fprintf( foutput, "свертка\t%d",-j1 );
		}
	...
	for( j0 = 1; j0 <= nnonter; ++j0 ){
		if( temp1[++j1] ) fprintf( foutput, "\t%s\tпереход\t%d\n", symnam( j0+NTBASE), temp1[j1] );
		}
	...
	*/
	...
	PLOOP(1,i){
		if( !(levprd[i] & REDFLAG) ){
			++j;
			if( foutput != NULL ){
				fprintf( foutput, "правило не свернуто: %s\n", writem( prdptr[i] ) );
				}
			}
		levprd[i] = *prdptr[i] - NTBASE;
		}
	if( j ) fprintf( stdout, "%d правил не свернуто\n", j );
	}
