#ifndef lint
/*NOXSTR*/
static char rcsid[] = "$Header: c03.c,v 2.2 86/11/26 17:27:56 dmitry Exp $" ;
/*YESXSTR*/
#endif  lint
 ...
 *
 *      $Log:	c03.c,v $
 *	Revision 2.2  86/11/26  17:27:56  dmitry
 *	Исправлена ошибка -
	...
	if (o != LBRACE) {
		...
		if (ssym->hclass!=tagkind)
			error(ediag( "Bad structure/union/enum name",
				     "неверное имя для struct/union/enum"));
		peeksym = o;
	} else {
		ds = defsym;
		mosflg = 0;
		...
		if (strp->ssize)
			error(ediag( "%s redeclared",
				     "%s несколько раз определено"), ssym->name);
		strp->ssize = elsize;
		*memlist++ = NULL;
	...
	if ((type&XTYPE) == FUNC) {
		...
		if ((skw!=EXTERN && skw!=TYPEDEF) && absname==NULL)
			error(ediag( "Bad func. storage class",
				     "неверный класс памяти для функции"));
	}
	if (!(dsym->hclass==0
	...
	if (dsym->hclass && (dsym->htype&TYPE)==STRUCT && (type&TYPE)==STRUCT)
		if (dsym->hstrp != tptr->hstrp) {
			error(ediag( "Warning: structure redeclaration",
				     "предупреждение: структура уже определена"));
			nerror--;
		}
	dsym->htype = type;
	if (tptr->hstrp)
	...
	/*      }                       */
		...
		if (type!=INT)
			error(ediag( "Illegal enumeration %s",
				     "неправильное перечислимое %s"), dsym->name);
		dsym->hclass = ENUMCON;
		dsym->hoffset = offset;
		if (isinit)
