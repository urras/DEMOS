#ifndef lint
/*NOXSTR*/
static char rcsid[] = "$Header: c01.c,v 2.2 86/11/26 17:26:42 dmitry Exp $" ;
/*YESXSTR*/
#endif  lint
 ...
 *
 *      $Log:	c01.c,v $
 *	Revision 2.2  86/11/26  17:26:42  dmitry
 *	Исправлена ошибка -
 ...
 *      Последнее изменение: 23.12.85 / FLEXNAME (Паремский М.В.)
 */
...
build(op)
{
	...
	if ((dope&BINARY)!=0) {
		...
		/*  t2 = p2->type;                 */
		/* P3: Загадочное исправление, внесенное, видимо, Марком. */
        	if ((p2=(*--cp))!=0) {
        		p2 = chkfun(disarray(p2));
        		t2 = p2->type;
        	} else
			t2=INT;
	}
	...
	case CAST:
		if ((t1&XTYPE)==FUNC || (t1&XTYPE)==ARRAY)
			error(ediag("Disallowed conversion",
				    "непредусмотренное преобразование"));
	...
	case QUEST:
		if (p2->op!=COLON)
			error(ediag("Illegal condition",
				    "неправильное условное выражение"));
		else
			if (fold(QUEST, p1, p2))
				return;
	...
	case ARROW:
		if (p2->op!=NAME || p2->tr1->hclass!=MOS) {
			error(ediag("Illegal structure ref",
				    "неправильная ссылка на структуру"));
			*cp++ = p1;
			return;
		}
		if( strucflg ) {
	...
	if (t1==STRUCT || t2==STRUCT) {
		...
		if( t2 == CHAR && ciflg ){
		    /* Это левая реализация предупреждения. Леонтьев С.Е. */
