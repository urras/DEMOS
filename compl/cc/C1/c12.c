#ifndef lint
/*NOXSTR*/
static  char    rcsid[] = "$Header: c12.c,v 2.2 88/12/26 20:28:05 dvolodin Exp $" ;
/*YESXSTR*/
#endif lint
 ...
 *      $Log:	c12.c,v $
 * Revision 2.2  88/12/26  20:28:05  dvolodin
 * Восстановлены скобки, потерянные в 2.9 BSD.
 * 
 * Revision 2.1  86/12/08  20:33:19  dmitry
...
struct tnode *atree;
{
	...
	case ASMOD:
		...
		if ((op==MOD||op==ASMOD) && tree->type==DOUBLE) {
			error(ediag("Floating %% not defined",
				    "для вещественных операция %% не определена"));
			tree->type = INT;
		}
	case ULSH:
	case ASULSH:
...
struct tnode *at;
{
	...
	error(ediag("Unimplemented field operator",
		    "нереализованная операция над полями"));
	return(t);
}
...
#ifndef sparc
        	 /* P3: Sbrk(2) conflicts with calls to malloc()   */
        	 /* from inside of stdio in SunOS.                 */
		if (sbrk(1024) == -1)
#endif
		{
			error(ediag("Out of space-- c1",
				    "c1: не хватает памяти"));
			exit(1);
		}
		coremax += 1024;
	}
