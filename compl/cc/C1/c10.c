#ifndef lint
/*NOXSTR*/
static  char    rcsid[] = "$Header: c10.c,v 2.3 87/02/25 20:17:48 alex Exp $" ;
/*YESXSTR*/
#endif lint
 ...
 *
 *              $Log:	c10.c,v $
 * Revision 2.3  87/02/25  20:17:48  alex
 * Правлена небольшая ошибка by @VG.
 * 
 * Revision 2.2  86/12/25  18:19:15  alex
 * Введен аргумент "-d" для работы с "cdeb".
 * 
 * Revision 2.1  86/12/08  20:30:51  dmitry
 * Первый рабочий вариант для ДЕМОС 2
 * 
 */
...
{
	if (argc<4) {
		error(ediag("Arg count",
			    "плохое число аргументов"));
		exit(1);
	}
	while ( argc > 4 && argv[--argc][0]=='-' ) {
	    if( argv[argc][1] =='d' ) {        /* Есть флаг -d : 17.05.85 */
		DebFlag++;
		if( argv[argc][2] == 'F' )  DEBlang = (1<<14);
		}
	    else if ( argv[argc][1] == 'V' ) ovlyflag++;
	    else {
		error(ediag("Unknown key %s","Неизвестный ключ %s"),argv[4]);
	    }
	}
	if (freopen(argv[1], "r", stdin)==NULL) {
		error(ediag("Missing temp file",
			    "отсутствует временный файл"));
		exit(1);
	}
	if ((freopen(argv[3], "w", stdout)) == NULL) {
		error(ediag("Can't create %s",
			    "нельзя создать %s"), argv[3]);
		exit(1);
	}
#ifdef sparc
	...
	if (totspace >= (unsigned)56000) {
		error(ediag("Warning: possibly too much data",
			    "предупеждение: возможно слишком большая программа"));
		nerror--;
	}
	exit(nerror!=0);
}
...
again:
	...
	/*
	 * String moves: especially for PASCAL by @VG
	 */
	case BYTEASG:
		byteasg(tree);
		return(0);
	...
	case CALL:
		...
		if (cexpr(tree, regtab, reg)<0)
			error(ediag("Compiler botch: call",
				    "ошибка компилятора: call"));
		popstk(r);
		nstack -= nargs;
	...
	if (tree->type == STRUCT)
		error(ediag("Illegal operation on structure",
			    "недопустимая операция со структурой"));
	else if (tree->op>0 && tree->op<RFORCE && opntab[tree->op])
		error(ediag("No code table for op: %s",
			    "в таблице нет кода для операции: %s"), opntab[tree->op]);
	else
		error(ediag("No code table for op: %d",
			    "в таблице нет кода для операции: %d"), tree->op);
	return(reg);
}
...
struct table *table;
{
	 ...
	 */
	/* P3: 'opd&BINARY && ' добавлено Марком. */
	if (opd&BINARY && tree->tr1->type==CHAR && tree->tr2->type!=CHAR
...
loop:
	...
	preg:
		...
		if (r>nreg || r>=4 && tree->type==DOUBLE)
			error(ediag("Register overflow: simplify expression",
				    "не хватает регистров: упростите выражение"));
		printf("r%d", r);
		goto loop;
...
struct table *table;
{
	...
	if (p->op==STAR || p->op==PLUS) {
		if (recurf && reorder(&p->tr1, table, reg))
			*treep = p = optim(p);
		p1 = p->tr1;		/* P3: can be null */
	}
	if (p1 != NULL && p1->op==NAME) switch(p->op) {
		case ASLSH:
		case ASRSH:
		case ASSIGN:
...
struct tnode **treep;
{
	...
	if (opdope[p->op]&BINARY) {
		if (p->op==LOGAND || p->op==LOGOR)
			return(0);
		/* P3: earlier this call was out of BINARY check */
		p1 = sdelay(&p->tr2);
	}
	if (p1==0)
		p1 = sdelay(&p->tr1);
	...
	if (tree->op==STRASG) {
		...
		if (tree->op!=NAME && tree->op!=STAR) {
			error(ediag("Unimplemented structure assignment",
				    "неправильное присваивание структур"));
			return(0);
		}
		...
		nstack++;
		return(size*2);
	} else if (tree->op==BYTEASG) { /* @VG - for PASCAL */
		size = tree->mask;
		tree = tree->tr1;
		tree = strfunc(tree);
		if (tree->op!=NAME && tree->op!=STAR) {
			error(ediag("assignment of string illegal",
				    "неправильное присваивание строки"));
			return(0);
		}
		tree = tnode(AMPER, STRUCT+PTR, tree);
		tree = optim(tree);
...
struct tnode *atree;
{
	...
	case LONG:
		if (tree->op==FTOL) {
			...
			goto illinit;
		printf("%o; %o\n", (short)(lval>>16) & 0xFFFF, (short)lval & 0xFFFF);
		return;
	}
illinit:
	error(ediag("Illegal initialization",
