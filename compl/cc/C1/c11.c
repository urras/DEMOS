 ...
 *  C compiler
 *
 *    Изменение: 30.03.85 Флеров М.Н. и @VG
 *    Последнее изменение: 31.03.85 @VG
 *    Очередное "последнее" изменение: 10.04.85 (флаг CDEBFlag для отладки)
 *    И еще одно последнее изменение:  9.12.85: @VG: byteasg for PASCAL
 *    И еще одно последнее изменение:  1.03.86: @VG: FCON1 for PASCAL
 *    И еще одно последнее изменение: 15.03.86: @VG: XBRANCH & XNAME for PASCAL
 *
 *    $Log:	c11.c,v $
...
getree()
{
	...
	static CDEBFlag = 0;      /* делать ли отладочные вставки 10.04.85 */
	...
	case PROG:
		printf(".text\n");
		CDEBFlag = 0;      /* включить отладочные вставки 10.04.85 */
		break;

	case DATA:
		printf(".data\n");
		CDEBFlag = 1;      /* выключить отладочные вставки 10.04.85 */
		break;
	...
	xpr:
		...
		if (sp != &expstack[1]) {
			error(ediag("Expression input botch",
				    "плохой ввод для генерации выражения"));
			exit(1);
		}
		nstack = 0;
		*sp = optim(*--sp);
...
struct fasgn *atp;
{
	...
	if (tp->op != ASSIGN) {
		...
		error(ediag("Illegal structure operation",
			    "неправильная операция над структурой"));
		return;
	}
		...
		if (tp->tr1->op!=NAME && tp->tr1->op!=STAR
		 || tp->tr2->op!=NAME && tp->tr2->op!=STAR) {
			error(ediag("Unimplemented structure assignment",
				    "невозможно присваивание структур"));
			return;
		}
...
struct fasgn *atp;
{
	...
	if (tp->op != ASSIGN) {
		...
		error(ediag("illegal operation with string",
			    "неправильная операция над строкой"));
		return;
	}
	tp->tr2 = strfunc(tp->tr2);
	if (tp->tr1->op!=NAME && tp->tr1->op!=STAR
	 || tp->tr2->op!=NAME && tp->tr2->op!=STAR) {
		error(ediag("assignment of string illegal",
			    "невозможно присваивание строки"));
		return;
	}
	tp->tr1 = tnode(AMPER, STRUCT+PTR, tp->tr1);
	tp->tr2 = tnode(AMPER, STRUCT+PTR, tp->tr2);
...
decref(at)
{
	...
	if ((t & ~TYPE) == 0) {
		error(ediag("Illegal indirection",
			    "неправильное взятие значения по адресу"));
		return(t);
	}
