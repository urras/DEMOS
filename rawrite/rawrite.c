...
void main(void)
{
  char	 fname[MAXPATH];
  char	*buffer, *pbuf;
  int	 count, fdin, drive, head, track, status, spt, buflength, ns, wlen;

  puts("RaWrite 1.2z - Write disk file to raw floppy diskette\n");
  ctrlbrk(handler);
 ...
 */
   ...
   while ((count = read(fdin, buffer, buflength)) > 0 && !done) {
     pbuf = buffer;
     for (ns = 1; count > 0 && !done; ns+=wlen) {
       printf("Track: %02d  Head: %2d Sector: %2d\r", track, head, ns);
       if ((wlen = count) > WMAXLEN) wlen = WMAXLEN;
       status = biosdisk(WRITE, drive, head, track, ns, wlen, pbuf);

       if (status != 0) Error(status);

       count -= (wlen*SECTORSIZE);
       pbuf  += (wlen*SECTORSIZE);
     }
